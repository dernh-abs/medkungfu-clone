// Bronchiectasis Stem Cell Therapy project detail page content.
...
const coreAdvantagesZh = [
  {
    title: "全球首创——气道基底层干细胞定向修复",
    icon: Icons.globe,
    description:
      "支气管扩张症是一种慢性呼吸道疾病，特征是支气管永久性扩张和反复感染。传统治疗仅能控制症状，无法修复受损的气道结构。Airway Basal Stem Cells (BSCs) 采用患者自身的气道基底层干细胞，通过其独特的再生能力，直接修复受损的支气管上皮，从根源改善病情。",
  },
  {
    title: "临床验证——减少急性加重频率",
    icon: Icons.trendingUp,
    description:
      "临床研究显示，接受气道基底层干细胞治疗的患者，急性加重发作频率显著降低，住院次数减少，抗生素使用量下降。患者的肺功能指标（FEV1、FVC）得到稳定或改善，生活质量评分显著提升。",
  },
  {
    title: "精准靶向——修复纤毛清除功能",
    icon: Icons.microscope,
    description:
      "支气管扩张症的核心病理机制是纤毛清除功能障碍导致黏液积聚和反复感染。气道基底层干细胞能够分化为功能性纤毛细胞，恢复气道的自我清洁能力，打破\"感染-炎症-损伤\"的恶性循环。",
  },
  {
    title: "综合获益——多维度改善",
    icon: Icons.heartPulse,
...
const processStepsZh = [
  {
    ...
    title: "全面评估与筛选",
    description:
      "包括高分辨率CT、肺功能测试、痰液培养、血液检查等，确认诊断并评估病情严重程度。排除活动性感染和其他禁忌症。",
  },
  {
    num: "02",
    title: "干细胞采集与制备",
    description:
      "通过支气管镜获取患者自身的气道基底层干细胞样本，在GMP标准实验室中进行分离、培养和扩增，确保细胞质量和安全性。",
  },
  {
    num: "03",
    title: "个性化治疗方案制定",
    description:
      "根据患者的年龄、病情严重程度、合并症等因素，制定个性化的细胞剂量、给药途径和治疗周期方案。",
  },
  {
    num: "04",
    title: "干细胞移植治疗",
    description:
      "通过雾化吸入或支气管镜引导下的精准递送方式，将制备好的气道基底层干细胞送达病变部位，实现局部高浓度治疗。",
  },
  {
    num: "05",
    title: "术后监测与康复",
...
const comparisonRowsZh: { aspect: string; traditional: string; stemCell: string }[] = [
  {
    aspect: "治疗原理",
    traditional: "控制症状，无法修复结构",
    stemCell: "再生修复，从根源改善",
  },
  {
    aspect: "急性加重频率",
    traditional: "频繁发作，难以控制",
    stemCell: "显著减少，趋于稳定",
  },
  {
    aspect: "肺功能变化",
...
const faqsZh = [
  {
    q: "什么是支气管扩张症？",
    a: "支气管扩张症是一种慢性呼吸道疾病，特征是支气管永久性异常扩张，导致黏液清除障碍、反复呼吸道感染和慢性炎症。常见症状包括慢性咳嗽、大量咳痰、反复肺部感染和呼吸困难。",
  },
  {
    q: "气道基底层干细胞治疗与传统治疗有何不同？",
    a: "传统治疗主要针对症状控制（如抗生素、祛痰药、支气管扩张剂），无法修复受损的气道结构。气道基底层干细胞治疗则利用患者自身的干细胞再生能力，直接修复受损的支气管上皮和纤毛结构，从根源改善病情，打破疾病进展的恶性循环。",
  },
  {
    q: "治疗过程是否安全？",
    a: "是的。该治疗使用患者自身的细胞，避免了免疫排斥反应。所有操作均在严格的质量控制下进行，符合GMP标准。临床试验和实际应用数据显示，该治疗方法具有良好的安全性和耐受性，不良反应轻微且可控。",
  },
  {
    q: "治疗后多久能看到效果？",
    a: "大多数患者在治疗后3-6个月内开始感受到明显改善，包括咳嗽减少、痰量降低、呼吸困难减轻等。肺功能的客观指标（如FEV1、DLCO）通常在6-12个月时达到最佳改善效果。长期随访显示，疗效可持续数年。",
  },
  {
    q: "哪些患者适合接受这种治疗？",
